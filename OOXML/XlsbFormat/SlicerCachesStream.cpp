 ...
 */
#include "SlicerCachesStream.h"

#include "Biff12_unions/SLICERCACHE.h"
...
const bool SlicerCachesStream::loadContent(BinProcessor& proc)
{	
	while (true)
	{
            ...
            switch(type)
            {
                case rt_BeginSlicerCache:
                {
                    if (proc.optional<SLICERCACHE>())
                    {
                        m_SLICERCACHE = elements_.back();
                        elements_.pop_back();
                    }
                }break;
