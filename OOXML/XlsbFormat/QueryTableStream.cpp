 ...
 */
#include "QueryTableStream.h"

#include "Biff12_records/CommonRecords.h"
#include "Biff12_unions/QSI.h"
...
const bool QueryTableStream::loadContent(BinProcessor& proc)
{	
	while (true)
	{
            ...
            switch(type)
            {
                case rt_BeginQSI:
                {
                    if (proc.optional<QSI>())
                    {
                        m_QSI = elements_.back();
                        elements_.pop_back();
                    }
                }break;
