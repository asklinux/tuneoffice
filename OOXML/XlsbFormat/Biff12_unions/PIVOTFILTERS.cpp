...
namespace XLSB
{
    ...
    BaseObjectPtr PIVOTFILTERS::clone()
    {
        return BaseObjectPtr(new PIVOTFILTERS(*this));
    }

    //PIVOTFILTERS = BrtBeginFilters *BrtFilter BrtEndFilters
    const bool PIVOTFILTERS::loadContent(BinProcessor& proc)
    {
        ...
        auto count = proc.repeated<Filter>(0, 0);
        while(count > 0)
        {
