...
namespace BinXlsxRW{
	...
	void ChartWriter::toXlsx(OOX::Spreadsheet::CXlsx& oXlsx)
	{
		...
		//Table
		//todo table for multiple sheets or if series are spread across sheets
            ...
            for(size_t j = 0; j < aIndexesCell.size(); ++j)
			{
				...
				if(pCell->m_oValue.IsInit())
				{
					...
					else if (false == pCell->m_oStyle.IsInit())
					{
						...
						{
							//like in CsvReader - suspicious code
							WCHAR *pEndPtr;
	...
	OOX::Spreadsheet::CCell* ChartWriter::parseCreateCell(const int& nRow, const int& nCol, const std::wstring& val, std::wstring format)
	{
		...
		//for now add as is, shared string after table write
