...
namespace PPTX
{
	...
	{		
		...
		ColorBase::ColorBase(const ColorBase& oSrc)
		{
			*this = oSrc;
		}
		ColorBase& ColorBase::operator=(const ColorBase& oSrc)
		{
		...
		DWORD ColorBase::GetRGBA(DWORD RGBA) const
		{
			return ApplyModifiers(ctRGBA);
		}
		DWORD ColorBase::GetARGB(DWORD ARGB) const
		{
			return ApplyModifiers(ctARGB);
		}
		DWORD ColorBase::GetBGRA(DWORD BGRA) const
		{
			return ApplyModifiers(ctBGRA);
		}
		DWORD ColorBase::GetABGR(DWORD ABGR) const
		{
			return ApplyModifiers(ctABGR);
		}
		DWORD ColorBase::GetRGBColor(NSCommon::smart_ptr<PPTX::Theme>& _oTheme, NSCommon::smart_ptr<PPTX::Logic::ClrMap>& _oClrMap, DWORD ARGB)
		{
			return GetARGB(ARGB);
		}
		void ColorBase::SetRGB(const BYTE& R, const BYTE& G, const BYTE& B)
		{
			red = R;
			green = G;
			blue = B;
		...
		const int ColorBase::HexChar2Int(const char value)
		{
			if (value >= '0' && value <= '9')
				return value - '0';
			if (value >= 'a' && value <= 'f')
		...
		const int ColorBase::HexString2Int(const std::wstring& value)
		{
			int summa = 0;
		...
		void ColorBase::SetHexString(const std::wstring& val)
		{
			if (val.length() == 6)
			{
		...
		void ColorBase::ReadModsFromPPTY(NSBinPptxRW::CBinaryFileReader* pReader)
		{
