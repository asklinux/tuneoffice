 ...
 */
	...
	function asc_CAutoFilterInfo() {
		...
		this.tableName = null;
		this.isApplyAutoFilter = false;   // Clear filter button: false - disabled, true - pressed button
		this.isAutoFilter = false;  // Autofilter button (also affects formatTable and Sort). Possible states:
		// - null - we are at intersection with table (but not completely inside it)
		// - true/false - when we are completely inside table or outside it (true/false depending on whether filter is applied or not)
		this.isSlicerAdded = false;// flag added for special case - block delete filter button for format table with slicer
	}
