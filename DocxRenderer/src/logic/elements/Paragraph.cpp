...
namespace NSDocxRenderer
{
	...
	void CParagraph::ToXml(NSStringUtils::CStringBuilder& oWriter) const
	{
		...
		if (m_dLineHeight > 0)
		{
			...
			oWriter.WriteString(L"\" w:lineRule=\"exact\""); // exact - precise line height
		}
		...
		oWriter.WriteString(L"/>"); // end of w:ind
		...
		case tatByLeft:
			...
			break;
		case tatUnknown:
		default: // default is left alignment - nothing needs to be added
			break;
		}
