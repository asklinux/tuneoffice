...
namespace MetaFile
{
	...
	void CWmfParser::PlayFile()
	{
		...
		while (!CheckError())
		{
			...
			m_unRecordSize = unSize * 2; // Size is specified in WORD
			...
			// Skip extra bytes that might have been in the record
	...
	void CWmfParser::Read_META_BITBLT()
	{
		...
		else
		{
			if (m_pOutput)
			{
				...
				// TODO: Implement Bitmap16 reading
			}
			else
			{
	...
	void CWmfParser::Read_META_EXTTEXTOUT()
	{
		...
		{
			if (shStringLength & 1) // If length is odd, skip 1 byte because of 2-byte alignment
				m_oStream.Skip(1);
