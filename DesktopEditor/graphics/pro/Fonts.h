...
namespace NSFonts
{
	...
	public:
		...
		{
			return (
				m_wsFontName == pFontInfo->m_wsFontName && m_wsStyle == pFontInfo->m_wsStyle && m_wsFontPath == pFontInfo->m_wsFontPath && m_bItalic == pFontInfo->m_bItalic &&
		...
		static inline bool IsStyleRegular(const std::wstring& style)
		{
			if (style == L"Regular" ||
				style == L"Roman")
				return true;
		...
		static inline bool IsStyleItalic(const std::wstring& style)
		{
			if (style == L"Italic" ||
				style == L"Oblique")
				return true;
		...
		static inline bool IsStyleBold(const std::wstring& style)
		{
			if (style == L"Bold")
				return true;
			return false;
		}
		static inline bool IsStyleBoldItalic(const std::wstring& style)
		{
			if (style == L"Bold Italic" ||
				style == L"Bold Oblique" ||
				style == L"BoldItalic" ||
				style == L"BoldOblique")
				return true;
			return false;
	...
	public:
		...
		std::wstring m_wsFontPath; // Path to font file
		long m_lIndex;			   // Font index in file (if file contains more than 1 font)
		...
		SHORT m_shCapHeight;	// Height of letter 'H' (uppercase)
