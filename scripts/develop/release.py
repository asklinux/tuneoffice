...
def protect_brunch(branch, repo, strict = False):
  ...
  command += team + '"]}} | gh api -X PUT repos/TUNEOFFICE/' + repo + '/branches/' + branch + '/protection --input -'
...
base.cmd_in_dir('../../', 'python', ['configure.py', '--branding', 'tuneoffice', '--branding-url', 'https://github.com/TUNEOFFICE/tuneoffice.git', '--branch', branch_from, '--module', 'core desktop builder server mobile', '--update', '1', '--update-light', '1', '--clean', '0'])

# parse configuration
config.parse()

base.git_update('tuneoffice')
...
repositories['tuneoffice'] = [True, False]
