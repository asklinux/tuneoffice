...
def make(solution=""):
  ...
  if config.check_option("module", "builder") and base.is_windows() and "tuneoffice" == config.branding():
    # check branding libs
    if (config.option("branding-name") == "tuneoffice"):
      for platform in platforms:
        if not platform in config.platforms:
          continue
        ...
        core_lib_branding_dir = os.getcwd() + "/../core/build/tuneoffice/lib/" + platform + base.qt_dst_postfix()
  ...
  if config.check_option("module", "builder") and "tuneoffice" == config.branding():
    for platform in platforms:
      if not platform in config.platforms:
        continue
